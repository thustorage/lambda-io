...
#define verbose(env, fmt, ...) lio_pr(info, fmt, ##__VA_ARGS__)
...
static int check_cfg(struct bpf_verifier_env *env)
{
	...
	insn_state = env->cfg.insn_state = calloc(insn_cnt, sizeof(int));
	if (!insn_state)
		return -ENOMEM;

	insn_stack = env->cfg.insn_stack = calloc(insn_cnt, sizeof(int));
	if (!insn_stack) {
		kvfree(insn_state);
		return -ENOMEM;
	}
...
static void mark_reg_unknown(struct bpf_verifier_env *env,
			     struct bpf_reg_state *regs, u32 regno)
{
	if (regno >= MAX_BPF_REG) {//WARN_ON(regno >= MAX_BPF_REG)) {
...
#define check_map_access(...) (-ENOTSUPP)
...
// yz
#define reg_may_point_to_spin_lock(...) false
...
#define try_match_pkt_pointers(...) false
...
{
	bool pop_log = false; //!(env->log.level & BPF_LOG_LEVEL2);
	struct bpf_verifier_state *state = env->cur_state;
	struct bpf_insn *insns = env->prog->insnsi;
	struct bpf_reg_state *regs;
	int insn_cnt = env->prog->len;
...
patch_call_imm:
		...
		{
			s64 offset = fn->func - __bpf_call_base;
			lio_pr(info, "__bpf_call_base=%p", __bpf_call_base);
			if ((s64)(s32)offset != offset) {
				assert(0);
			}
		}
...
static bool bpf_jit_needs_zext()
{  
	// yz
	return false;
}
	      ...
	      union bpf_attr *uattr)
{
	 ...
	 */
	env = calloc(1, sizeof(struct bpf_verifier_env));
	if (!env)
		return -ENOMEM;
	// log = &env->log;
	...
	// yz add begin
	env->visited_bitmap = calloc(1, len);
	...
	env->insn_aux_data =
		calloc(len, sizeof(struct bpf_insn_aux_data));
	ret = -ENOMEM;
	...
	env->explored_states = calloc(state_htab_size(env),
